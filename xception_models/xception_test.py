...
class XceptionTest(tf.test.TestCase):
  ...
  def testBuild(self):
    ...
    with self.test_session():
      ...
      logits, end_points = xception_models.xception(inputs, num_classes)
  ...
  def testEvaluation(self):
    ...
    with self.test_session():
      ...
      logits, _ = xception_models.xception(eval_inputs, is_training=False)
  ...
  def testForward(self):
    ...
    with self.test_session() as sess:
      inputs = tf.random_uniform((batch_size, height, width, 3))
      logits, _ = xception_models.xception(inputs)
