...
def data_augmentation_img(raw_img):
    # new_path = raw_path.replace('img_data', 'img_augmen_data')
    #
    # raw_img = Image.open(raw_path)
    ...
    # 镜像
    jx_1 = raw_img.transpose(Image.FLIP_LEFT_RIGHT)
    jx_1 = jx_1.resize((224, 224))
    jx_1 =np.asarray(jx_1)
    jx_2 = raw_img.transpose(Image.FLIP_TOP_BOTTOM)
    jx_2 = jx_2.resize((224, 224))
    ...
    if bool(random.getrandbits(1)):
        ...
        fs = fs_2
    # 随机切割
    cc = transforms.CenterCrop(int(224))
    sq_1 = cc(raw_img)
    sq_1 = np.asarray(sq_1)
    rc = transforms.RandomCrop(int(224))
    sq_2 = rc(raw_img)
